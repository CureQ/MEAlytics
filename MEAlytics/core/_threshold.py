...
def fast_threshold(data, parameters):
    ...
    for j in loopvalues:
        ...
        if not (
            np.min(data[j * stepsize : (j + 1) * stepsize])
            < (-1 * parameters["standard deviation multiplier"] * std)
            or np.max(data[j * stepsize : (j + 1) * stepsize])
            > (parameters["standard deviation multiplier"] * std)
        ):
