...
def burst_detection(
    ...
    if spikedata.shape[0] >= parameters["minimal amount of spikes"]:
        ...
        output = sns.displot(
            ...
            kde_kws={
                "gridsize": 100,
                "bw_adjust": parameters["burst detection kde bandwidth"],
            },
        )
        ...
        for i in range(0, len(x)):
            ...
            if upper_boundary > len(x):
                upper_boundary = len(x)

            # Check if this value is higher than its specified amount of neighbours
            if np.all(y[i] > y[lower_boundary:i]) and np.all(
                y[i] > y[i + 1 : upper_boundary]
            ):
        ...
        if len(peaks) > 2:
            ...
            for j in range(len(peaks)):
                dist_from_default = abs(
                    x[peaks[j]] - parameters["default interval threshold"]
                )
                if dist_from_default < 0.9 * parameters["default interval threshold"]:
                    if dist_from_default < abs(
                        peak1 - parameters["default interval threshold"]
                    ):
                        peak1 = peaks[j]
                        found_peak1 = True
            ...
            for j in range(len(peaks)):
                dist_from_default = abs(
                    x[peaks[j]] - parameters["max interval threshold"]
                )
                if x[peaks[j]] > parameters["default interval threshold"]:
                    if dist_from_default < abs(
                        peak2 - parameters["max interval threshold"]
                    ):
                        peak2 = peaks[j]
                        found_peak2 = True
        ...
        if plot_electrodes:
            ...
            for peak in peaks:
                peakneighbours = np.arange(
                    np.max([peak - smallerneighbours, 0]),
                    np.min([peak + smallerneighbours + 1, len(x)]),
                )
        ...
        if plot_electrodes:
            ...
            rawburstplot.title.set_text(
                f"Well {well} - Electrode {electrode}, bursts detected: {len(burst_cores)}, {thresholdtext}"
            )
    ...
    # If there are not enough spikes for the burst detection, wel fill the figures and output arrays with nothing
        ...
        if plot_electrodes:
            ...
            fillerplot2.title.set_text(
                f"No burst detection possible for well {well}, electrode {electrode} - not enough values"
            )
