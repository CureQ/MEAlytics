...
from MEAlytics.core._isi_distance import default_thresh, isi_distance
...
def electrode_features(well, parameters):
    ...
    (
        ...
        median_spike_amplitude,
        cv_spike_amplitude,
    ) = [], [], [], [], []

    # Calculate which electrodes belong to the well
    electrodes = np.arange(
        (well - 1) * parameters["electrode amount"],
        well * parameters["electrode amount"],
    )
    ...
    for electrode in electrodes:
        ...
        with h5py.File(output_hdf_file, "r") as f:
            spikedata = f[
                f"spike_values/well_{well_nr}_electrode_{electrode_nr}_spikes"
            ]
            spikedata = spikedata[:]
            burst_cores = f[
                f"burst_values/well_{well_nr}_electrode_{electrode_nr}_burst_cores"
            ]
            burst_cores = burst_cores[:]
            burst_spikes = f[
                f"burst_values/well_{well_nr}_electrode_{electrode_nr}_burst_spikes"
            ]
...
-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
        ...
        firing_frequency = spike / (
            parameters["measurements"] / parameters["sampling rate"]
        )
        ...
        if len(burst_cores) > 0:
            MAD_spikes_per_burst.append(
                np.mean(np.absolute(spikes_per_burst - np.mean(spikes_per_burst)))
            )
        else:
        ...
        if spikedata.shape[0] < 2:
            ...
            cv_spike_amplitude.append(
                np.std(np.abs(spikedata[:, 1])) / np.mean(np.abs(spikedata[:, 1]))
            )
    ...
    if parameters["remove inactive electrodes"]:
        ...
        features_df = features_df[
            features_df["Mean Firing Rate"] > parameters["activity threshold"]
        ]
    ...
    if parameters["remove inactive electrodes"]:
        features_df.insert(
            2, "Active_electrodes", [active_electrodes] * len(features_df)
        )
    return features_df
...
def electrode_pair_features(parameters, save_data=True):
    ...
    time_df = np.arange(
        0, int(parameters["measurements"]) / int(parameters["sampling rate"])
    )
    ...
    with h5py.File(output_hdf_file, "r") as f:
        for well in wells:
            ...
            distance_matrix = np.full(
                (len(electrodes), len(electrodes)), np.nan
            ...
            for electrode_x, electrode_y in unique_pairs_electrodes:
                ...
                if (spiketrain_x is None or spiketrain_x.size == 0) or (
                    ...
                    if (
                        parameters["synchronicity method"] == "adaptive_ISI_distance"
                        or parameters["synchronicity method"]
                        == "adaptive_SPIKE_distance"
                    ):
                        # Calculate automatic threshold
                        spiketrains = [
                            np.asarray(spiketrain_x),
                            np.asarray(spiketrain_y),
                        ]
                    ...
                    if (
                        parameters["synchronicity method"] == "adaptive_ISI-distance"
                        or parameters["synchronicity method"] == "ISI_distance"
                    ):
                        distance_value, _ = isi_distance(
                            spiketrain_x, spiketrain_y, start_time, end_time, threshold
                        )

                    # Calculate synchronicity for SPIKE-distance
                    else:
                        distance_value, _ = spike_distance(
                            spiketrain_x,
                            spiketrain_y,
                            start_time,
                            end_time,
    ...
    if save_data:
        with h5py.File(output_hdf_file, "a") as f:
            f.create_dataset(
                f"synchronicity_values/{method}_well", data=mean_distance_df
            )
            for well, matrix in matrices.items():
                f.create_dataset(
                    f"synchronicity_values/{method}_electrodes_pair/matrix_well_{well}",
                    data=matrix,
                )
...
def well_features(well, parameters):
    ...
    for electrode in electrodes:
        with h5py.File(output_hdf_file, "r") as f:
            ...
            spikedata = spikedata[:]
            burst_cores = f[
                f"burst_values/well_{well}_electrode_{electrode}_burst_cores"
            ]
            burst_cores = burst_cores[:]
            burst_spikes = f[
                f"burst_values/well_{well}_electrode_{electrode}_burst_spikes"
            ]
            burst_spikes = burst_spikes[:]
    ...
    (
        ...
        network_burst_core_duration,
        network_IBI,
        NB_NBc_ratio,
        NB_firingrate,
        NB_ISI,
        mean_spikes_per_network_burst,
    ) = [], [], [], [], [], [], [], []
    (
        nIBI_var,
        nIBI_CV,
        ...
        portion_spikes_in_nbs,
        portion_bursts_in_nbs,
    ) = [], [], [], [], [], [], [], [], [], []
...
-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    if len(network_cores) == 0:
        ...
        for network_burst in range(len(network_cores)):
            ...
            for channel in range(len(spikedata_list)):
                ...
                for spike in range(len(spikedata_list[channel])):
                    ...
                    if (
                        network_cores[network_burst, 2]
                        <= spikedata_list[channel][spike][0]
                        <= network_cores[network_burst, 3]
                    ):
                        nb_spikes.append(spikedata_list[channel][spike][0])
            # Calculate the NB firing rate (spikes / durations)
            single_nb_firingrate.append(
                (len(nb_spikes))
                / (network_cores[network_burst, 3] - network_cores[network_burst, 2])
            )
        ...
        ratios = []
        for i in range(len(network_cores)):
            ratio = (network_cores[i, 3] - network_cores[i, 2]) / (
                network_cores[i, 1] - network_cores[i, 0]
            )
            ratios.append(ratio)
        ...
        for network_burst in np.unique(participating_bursts[:, 0]):
            ...
            electrodes_participated = len(
                np.unique(
                    participating_bursts[participating_bursts[:, 0] == network_burst][
                        :, 1
                    ]
                )
            )
...
def feature_output(electrode_features, well_features):
    ...
    avg_electrode_features = pd.concat(
        [avg_electrode_features, well_features], axis=1, join="inner"
    )
...
def recalculate_features(
    outputfolder, well_amnt, electrode_amnt, electrodes, sampling_rate, measurements
):
    """
    ...
    for well in range(well_amnt):
        ...
        well_electrodes_mask = electrodes[
            well * electrode_amnt : (well + 1) * electrode_amnt
        ]
        ...
        # Combine the dataframes
        avg_electrode_features = pd.concat(
            [avg_electrode_features, well_features_df], axis=1, join="inner"
        )
    ...
    combined_df.to_csv(
        f"{os.path.join(outputfolder, os.path.basename(os.path.normpath(outputfolder)))}_Features.csv",
        index=False,
    )
