...
def spike_validation(
    ...
    spikeduration_samples = int(
        parameters["refractory period"] * parameters["sampling rate"]
    )
    ...
    time_seconds = np.arange(
        0, data.shape[0] / parameters["sampling rate"], 1 / parameters["sampling rate"]
    )
    ...
    if parameters["drop amplitude"] != 0:
        ...
        for j in loopvalues:
            ...
            # The spike does not happen too close to the beginning or end of the measurement, so we can validate it
                ...
                noise_left = data[
                    j - surrounding_noise_window_samples - exit_time : j - exit_time
                ]
                noise_right = data[
                    j + exit_time : j + surrounding_noise_window_samples + exit_time
                ]

                # Only use values between the threshold
                noise_surround = np.append(noise_left, noise_right)
                noise_surround = noise_surround[
                    (noise_surround > -threshold) & (noise_surround < threshold)
                ]

                # Calculate the RMS of the surrounding noise
                drop_amount = parameters["drop amplitude"] * (
                    np.sqrt(np.mean(noise_surround**2))
                )
                ...
                if data[j] > 0:
                    if not (
                        np.min(data[j - exit_time : j + exit_time + 1])
                        < (data[j] - drop_amount)
                    ):
                        spikes[j] = False
                else:
                    # For negative spikes
                    if not (
                        np.max(data[j - exit_time : j + exit_time + 1])
                        > (data[j] + drop_amount)
                    ):
                        spikes[j] = False
    ...
    if plot_electrodes:
        ...
        if plot_rectangles:
            ...
            for j in loopvalues:
                ...
                if spikes[j]:
                    ...
                    rawdataplot.add_patch(
                        Rectangle(
                            (time_seconds[j - exit_time], data[j]),
                            exit_time * (1 / parameters["sampling rate"]) * 2,
                            boxheight,
                            edgecolor="green",
                            facecolor="none",
                            linewidth=0.5,
                    ...
                    rawdataplot.add_patch(
                        Rectangle(
                            (time_seconds[j - exit_time], -threshold),
                            -(surrounding_noise_window),
                            threshold * 2,
                            edgecolor="violet",
                            facecolor="violet",
                            linewidth=0.5,
        ...
        # Plot layout
        rawdataplot.title.set_text(
            f"Well {well} - MEA electrode {electrode} - Threshold: {threshold} - Spikes detected before validation: {np.sum(spikes_before_validation)}, after: {np.sum(spikes)}"
        )
        ...
        while True:
            try:
                with h5py.File(output_hdf_file, "a") as f:
                    f.create_dataset(
                        f"spike_values/well_{well}_electrode_{electrode}_spikes",
                        data=spike_output,
                    )
                break
            except:
