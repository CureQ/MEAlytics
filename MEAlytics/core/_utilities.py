...
def rechunk_dataset(
    ...
    with h5py.File(fileadress, "r") as src, h5py.File(outputfile, "w") as dst:
        ...
        print(
            f"Rechunking and compressing dataset to shape: {new_chunks}, this will create a new file"
        )
            ...
            if isinstance(obj, h5py.Dataset):
                if name == dataset_to_rechunk:
                    if compression_method == "lzf":
                        ...
                        raise ValueError(
                            f"{compression_method} is not a valid compression method"
                        )
        ...
        print(
            f"Original size: {round(original_size, 2)} GB - New size: {round(new_size, 2)} GB"
        )
        print("Rechunking and compression succesful")
        return outputfile
