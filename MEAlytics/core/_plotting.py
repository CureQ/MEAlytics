...
def well_electrodes_kde(outputpath, well, parameters, bandwidth=1):
    ...
    for electrode in range(1, electrode_amnt + 1):
        if first_iteration:
            ...
            ax = fig.add_subplot(
                gs[electrode_amnt - electrode], sharex=axes[0], sharey=axes[0]
            )
        ...
        if len(spikedata[:, 0]) > 0:
            y = (
                FFTKDE(bw=bandwidth, kernel="gaussian")
                .fit(spikedata[:, 0])
                .evaluate(grid_points=np.arange(0, data_time, 0.001))
            )
...
def feature_boxplots(
    ...
    if well_amnt is not None:
        rainbow_colors = [
            f"#{np.random.randint(0, 0xFFFFFF):06X}" for _ in range(well_amnt)
        ]
    ...
    if colors == None:
        ...
        if len(labels) > len(colors):
            raise ValueError(
                f"You have supplied more labels ({len(labels)}) than there are colors available ({len(colors)}). Please supply colors manually using the 'colors' parameter."
            )
    else:
        if len(labels) > len(colors):
            raise ValueError(
                f"You have supplied more labels ({len(labels)}) than colors ({len(colors)}). Please supply more color values, or use the default colors ({len(defaultcolors)} colors)"
            )
                ...
                if show_datapoints:
                    for i, well in enumerate(wells):
                        ...
                        if discern_wells and (well_amnt is not None):
                            ...
                            color = "black"
                        plt.scatter(
                            x[0], temp_data[i], alpha=0.5, color=color, zorder=10, s=1
                        )
            # Remove NaNs
            plotdata = [d[~np.isnan(d)] for d in plotdata]
            bplot = ax.boxplot(
                plotdata, vert=True, labels=labels.keys(), patch_artist=True, widths=0.5
            )
...
def combined_feature_boxplots(
        ...
        for key in labels.keys():
            combined_labels[key] = (
                (
                    np.append(
                        combined_labels[key],
                        (np.array(labels[key])) + (original_rows * i),
                    )
                ).astype(int)
            ).tolist()
    ...
    # Plot the data
    pdf_path = feature_boxplots(
        dataframe,
        combined_labels,
        output_fileadress=output_fileadress,
        colors=colors,
        show_datapoints=show_datapoints,
        discern_wells=discern_wells,
        well_amnt=well_amnt,
    )
    return pdf_path


def features_over_time(
    folder, labels, div_prefix, output_fileadress, colors=None, show_datapoints=False
):
    """
    ...
    for featurefile in range(len(sorted_featurefiles)):
        sorted_dataframes.append(
            (
                sorted_featurefiles[featurefile][0],
                pd.read_csv(sorted_featurefiles[featurefile][1]),
            )
        )
    ...
    for i in range(len(features)):
        ...
        if features[i] not in not_features:
            ...
            for index, key in enumerate(labels.keys()):
                ...
                for pos, num in enumerate(nums):
                    ...
                    if show_datapoints:
                        ...
                        plt.scatter(
                            x=np.array([pos] * len(temp_data))
                            + np.random.uniform(-0.2, 0.2, len(temp_data)),
                            y=temp_data,
                            color=colors[index],
                            s=5,
                            alpha=0.5,
            ...
            plt.legend()
            plt.xticks(
                ticks=range(len(nums)), labels=graphlabels, rotation=45, ha="right"
            )
            plt.tight_layout()
            pdf.savefig()
            plt.close()
