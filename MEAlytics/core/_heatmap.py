...
def cmap_creation():
    ...
    smooth_cmap = mcolors.LinearSegmentedColormap.from_list(
        "smooth_heatmap", colours, N=256
    )
    return smooth_cmap
...
def data_prepper(h5_data, parameters, Vars, calculate_electrode_grid):
    time_df = np.arange(
        0,
        int(parameters["measurements"]) / int(parameters["sampling rate"]),
        1 / Vars["fps"],
    )
    ...
    with h5py.File(h5_data, "r") as file:
        ...
        for filename in sorted_matches:
            ...
            for frame_center in time_df:
                ...
                count = np.sum(
                    (timestamps_array >= window_start) & (timestamps_array < window_end)
                )
    ...
    precomputed_data = [
        reshape_wells(
            final_dataframe.iloc[frame], n_Wells, n_Electrodes, Size, electrode_grid
        )
...
def make_hm_img(Vars, Classes, Colour_classes):
    ...
    fig.suptitle("Total spikes per electrode", fontsize=14, color="white")
    i = 0
    vmax = df.max().max()

    for i, ax in enumerate(axs):
        well_values_flat = df.iloc[
            0, i * n_Electrodes : (i + 1) * n_Electrodes
        ].to_numpy()
...
def make_hm_max_activity(Vars, Classes, Colour_classes):
    ...
    fig.suptitle(
        "Highest amount of spikes in 1 second per electrode", fontsize=14, color="white"
    )
    i = 0
    vmax = df.max().max()
...
def create_placeholder_figure(Vars):
    ...
    title = fig.suptitle(
        "Spike rate (Spikes/Sec)", color="white", fontsize=16, weight="bold", y=0.95
    )
    title.set_path_effects([patheffects.withStroke(linewidth=3, foreground="black")])
    return fig
