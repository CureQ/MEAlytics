...
def network_burst_detection(
    wells, parameters, plot_electrodes=False, save_figures=True, savedata=True
):
    """
    ...
    Notes
    -----
    Instead of returning the results of the network burst detection using 'return', the function saves them at a specific file location using h5 files.
    """
    ...
    for well in wells:
        ...
        KDE_grid = np.arange(
            0,
            parameters["measurements"] / parameters["sampling rate"],
            (1 / parameters["sampling rate"]) * 1,
        )
        if len(spikes_for_kde) > 0:
            # Create the KDE using the KDEpy library
            y = (
                FFTKDE(bw=parameters["nbd kde bandwidth"], kernel="gaussian")
                .fit(spikes_for_kde)
                .evaluate(grid_points=KDE_grid)
            )
            x = KDE_grid
        ...
        if len(burst_spikes_for_kde) > 0:
            ...
            if threshold_method == "Yen" or threshold_method == "yen":
                ...
                raise ValueError(
                    f'"{threshold_method}" is not a valid thresholding method'
                )
            ...
            for i in range(len(network_burst_cores)):
                ...
                for channel in burst_cores_list:
                    ...
                    if channel_participates:
                        channels_participating += 1
                # Mark network burst cores that are too close to the start or end of the measurement
                too_close = (
                    network_burst_cores[i][0] < buffer
                    or network_burst_cores[i][1]
                    > (parameters["measurements"] / parameters["sampling rate"])
                    - buffer
                )
                ...
                for j in range(len(burst_cores_list)):
                    for burst in burst_cores_list[j]:
                        ...
                        if _overlap(network_burst_cores[i], burst):
                            ...
                            if burst[0] < outer_start:
                                outer_start = burst[0]
                            if burst[1] > outer_end:
                                outer_end = burst[1]
            ...
            for i in range(len(network_burst_cores) - 1):
                ...
                if _overlap(
                    network_burst_cores[i][2:4], network_burst_cores[i + 1][2:4]
                ):
        ...
        if savedata:
            with h5py.File(output_hdf_file, "a") as f:
                f.create_dataset(
                    f"network_values/well_{well}_network_bursts",
                    data=total_network_burst,
                )
                f.create_dataset(
                    f"network_values/well_{well}_participating_bursts",
                    data=participating_bursts,
                )
